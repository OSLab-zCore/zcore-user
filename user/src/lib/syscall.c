...
static inline int syscall(int num, ...) {
  ...
  asm volatile("syscall"
               : "=a"(ret)
               : "a"(num), "D"(a[0]), "S"(a[1]), "d"(a[2]), "r"(r10), "r"(r8)
               : "rcx", "r11", "memory");
  ...
  __asm__ __volatile__("addu $2,$0,%2 ; syscall"
                       : "=&r"(v0), "=r"(a3)
                       : "ir"(num), "0"(v0), "1"(a3), "r"(a0), "r"(a1), "r"(a2),
                         "r"(a3), "r"(a4), "r"(a5)
...
int sys_times(void) { return syscall(SYS_times); }

int sys_exec(const char *name, int argc, const char **argv) {
  return syscall(SYS_exec, name, argv, NULL);
}

int sys_open(const char *path, uint32_t open_flags) {
  const int AT_FDCWD = -100;
  const int mode = 0;
  return syscall(SYS_openat, AT_FDCWD, path, open_flags, mode);
}
...
int sys_readat(int fd, void *base, size_t len, size_t offset) {
  return syscall(SYS_readat, fd, base, len, offset);
}

int sys_writeat(int fd, void *base, size_t len, size_t offset) {
  return syscall(SYS_writeat, fd, base, len, offset);
}
...
int sys_fsync(int fd) { return syscall(SYS_fsync, fd); }
...
int sys_dup(int fd1, int fd2) { return syscall(SYS_dup3, fd1, fd2); }

// DO NOT USE THIS
int sys_sleep(unsigned int time) { return syscall(SYS_sleep, time); }

int sys_nanosleep(struct timespec *time) { return syscall(SYS_sleep, time); }
...
int sys_setup_async_call(int req_capacity, int comp_capacity, int worker_num,
                         struct async_call_info *info, size_t info_size) {
  return syscall(SYS_setup_async_call, req_capacity, comp_capacity, worker_num,
                 info, info_size);
}
