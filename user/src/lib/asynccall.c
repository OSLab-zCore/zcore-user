...
int async_call_buffer_init(int req_capacity, int comp_capacity, int worker_num,
                           struct async_call_buffer *buffer)
{
    struct async_call_info info;
    int ret = sys_setup_async_call(req_capacity, comp_capacity, worker_num, &info, sizeof(info));
    if (ret < 0)
    {
        cprintf("sizeof async_call_info %d \n", sizeof(info));
        return ret;
    }
